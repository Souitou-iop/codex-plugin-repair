...
脚本会先让你选择语言，然后立刻检查可能占用插件文件的常见进程，例如 Codex、`extension-host`、`codex-computer-use`。如果检测到 Codex Desktop，会先询问是否关闭；推荐输入 `y` 或 `yes` 先关闭它，避免文件被占用。不关闭也可以继续运行，但修复完成后需要重启 Codex Desktop。之后脚本会说明计划执行的操作，并等待你再次输入 `y` 或 `yes` 后才会开始修改。修改前会给 `~/.codex/config.toml` 创建带时间戳的备份；运行时会显示步骤进度，执行完成或遇到问题时，终端会输出提示。
